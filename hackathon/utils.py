...
def get_geojson(level=2):
    r = requests.get('https://play.dhis2.org/dev/api/26/organisationUnits.geojson?level=2',
                     auth=('admin', 'district'))
    return r


def get_geojson_4():
    r = requests.get('https://play.dhis2.org/dev/api/26/organisationUnits.geojson?level=4',
                     auth=('admin', 'district'))
    return r
...
def get_analytics(query=None):
    r = requests.get(
        'https://play.dhis2.org/release1/api/25/analytics.json?dimension=dx:YtbsuPPo010;l6byfWFUGaP;s46m5MS0hxu&dimension=pe:LAST_12_MONTHS&filter=ou:ImspTQPwCqd&displayProperty=NAME&skipMeta=true',
        auth=('admin', 'district'))
    return r.json()
